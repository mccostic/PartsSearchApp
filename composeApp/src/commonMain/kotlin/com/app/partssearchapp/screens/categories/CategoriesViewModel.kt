...
class CategoriesViewModel(params: CategoriesParams, private val partsDataService: PartsDataService,) :
        ...
        uiEvents
            ...
            .collect { event ->
                if (event.category.subcategories.isEmpty()) {
                    ...
                    updateState {
                        copy(
                            expandedCategoryId = if (expandedCategoryId == event.category.id) {
                                null
                            } else {
                                event.category.id
                            }
    ...
    private suspend fun toggleCategoryHandler() {
        uiEvents
            ...
            .collect { event ->
                updateState {
                    copy(
                        expandedCategoryId = if (expandedCategoryId == event.categoryId) {
                            null
                        } else {
                            event.categoryId
                        }
